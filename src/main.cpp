    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Widely spaced (x, y) waypoints. Will be fitted to a spline
          vector<double> ptsx;
          vector<double> ptsy;
          ...
          if (prev_size < 2) {
            // 2-points which make the path tangent to the car
            ...
            // Use previous paths endpoint as new start point
          ...
          // In Frenet co-ordinates, add every 30m apart
          ...
          for (int i = 0; i < ptsx.size(); ++i) {
            // Transform car reference angle to 0 degrees
          ...
          // (x,y) points to be used for the planner
          vector<double> next_x_vals;
          vector<double> next_y_vals;

          // Add previous path points from last time
          for (int i = 0; i < previous_path_x.size(); ++i) {
            next_x_vals.push_back(previous_path_x[i]);
            next_y_vals.push_back(previous_path_y[i]);
          }

          // Break spline so that we can travel at the desired reference velocity
          ...
          // Fill up rest of path after filling previous untraversed points
          for (int i = 1; i <= 50 - previous_path_x.size(); ++i) {
            double N = (target_dist / (0.02 * ref_vel/ 2.24)); // 2.4 to convert to m/s from mph
            ...
            // Transform back to normal coordinates to be used by path planner
